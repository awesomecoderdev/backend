<x-app-layout>
    @section('head')
        <title>{{ __('Edit Invoice') }} {{ config('settings.separator') }} {{ __(config('settings.title')) }}</title>
    @endsection
    <x-content>
        <div class="mt-10 sm:mt-0">
            <div class="md:grid md:grid-cols-3 md:gap-6">
                <div class="mt-5 md:col-span-6 md:mt-0">
                    <form action="{{ route('users.update', $invoice) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="overflow-hidden sm:rounded-md border border-gray-200 dark:border-slate-800 ">
                                ...
                                <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <input type="text" name="first_name" id="first_name"
                                            ...
                                            value="{{ old('first_name') ?? $invoice->first_name }}"
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <input type="text" name="last_name" id="last_name" autocomplete="family-name"
                                            value="{{ old('last_name') ?? $invoice->last_name }}"
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <input type="text" name="email" id="email" autocomplete="email"
                                            readonly value="{{ $invoice->email }}" disabled
                            ...
                            <div class="bg-gray-50  dark:bg-gray-700/10 px-4 py-3 text-right sm:px-6">
                                <button type="submit"
                                    class="inline-flex justify-center rounded-md border border-transparent bg-primary-600 py-2 px-4 text-sm font-medium text-white shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2">{{ __('Update') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
